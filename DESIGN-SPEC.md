...
- **Freshness — two clocks, told apart:** a global "checked n min ago" in the header is *our* pipeline (collector runs every 5 min); each card's "reported {clock}" is the *border's own* time. Our pipeline stale > 10 min shows the amber warning strip. The two clocks fail *independently*: if the border's own number (feed_updated_at) hasn't refreshed in > 60 min — a threshold set well past CBP's hourly cadence and its occasional future-stamps, so we never cry wolf — that card carries an amber on-grid "official {agency} data hasn't updated in {n}" strip under the hero (the agency — cbp / cbsa — is named from the reading's source so the blame lands on their stopped clock, not ours), and that reading is barred from winning or being tagged in the fastest box (same bar as a closed lane or a pipeline gap). A stale source number must never read as live even when our pipeline is healthy (jul 2026 trust fix).
- **Alert row:** the top incident per card at a glance, UPPERCASE on a signal fill (amber = lanes affected, red = full closure), separated by a 2px rule. When a card stacks more than one, the strip becomes a tap-to-open list (glove-sized 56px summary, +/− marker inheriting the strip's text color) that reveals every other alert on its own signal fill — "+n more" must always lead to what those alerts are, never a dead-end count. Without an incident: "no incidents" in muted, unfilled. If our alert collection isn't current (> 15 min), we say "we can't check road incidents right now" rather than claim none. Text wraps to stay fully readable — never clipped; a safety alert you can't finish reading fails the trust rule, which outranks the calm ideal.
